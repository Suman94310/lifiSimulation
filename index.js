window.onload = ()=>{
    ...
    let carsProp = [
        ...
        {
            velocity:{x:speed,y:speed/2},
            position:{x:110,y:window.innerHeight*2/3-70},
            angle: 1,
            road: -1
        }
    ]
    ...
    road.position.set(0,window.innerHeight*2/3)
    road.width = window.innerHeight*2/3
    road.height = window.innerWidth*2/3
    road.rotation = -1.57
    ...
    for(let i=0; i<carsProp.length; i++){
        ...
        cars[i].height = window.innerWidth*2/3/30
        cars[i].width = window.innerWidth*2/3/42
        cars[i].anchor.x = 0.5
    ...
    app.ticker.add((delta)=>{
        ...
        if(road1.length >= road2.length){
            if(road1.length){
                carsProp[road1[0]].velocity = {x:speed, y:speed/2}
            }
            if(road2.length && carsProp[road2[0]].road && cars[road2[0]].x > window.innerWidth*2/3/2 -200){
                carsProp[road2[0]].velocity = {x:0, y:0}
            }
        }
        else{
            if(road2.length){
                carsProp[road2[0]].velocity = {x:speed, y:speed/2}
            }
            if(road1.length && carsProp[road1[0]].road && cars[road1[0]].x > window.innerWidth*2/3/2 -200){
        ...
        for(let i=0; i<len; i++){
            ...
            if(cars[i].x >= window.innerWidth*2/3/2-60 && cars[i].rotation*carsProp[i].road > 1.55*carsProp[i].road && carsProp[i].road){
    ...
    window.onclick = (e)=>{
        let flag = 1
        if(e.clientY < window.innerHeight*2/3/2){
            for(let k=0; k<len; k++){
            ...
            if (flag){
                carsProp.push({
                    velocity:{x:speed ,y:speed/2},
                    position:{x:60,y:40 + Math.random()*window.innerHeight*2/3/20},
                    angle: 2,
                    road:1
                })
                current+=1
            ...
            if (flag){
                carsProp.push({
                    velocity:{x:speed,y:speed/2},
                    position:{x:110,y:window.innerHeight*2/3-70 - Math.random()*window.innerHeight*2/3/20},
                    angle: 1,
                    road:-1
                })
                current+=1
        ...
        if (flag){
            const carTexture = PIXI.Texture.from("./car.svg")
            cars.push(new PIXI.Sprite(carTexture))
            cars[cars.length-1].height = window.innerWidth*2/3/30
            cars[cars.length-1].width = window.innerWidth*2/3/42
